"""
DeepFace + Emotion2Vec MLP Fusion Demo

DeepFace (calibrated neutral + happy) as the visual branch.
Emotion2Vec large (raw, no calibration) as the audio branch.
MLP Learned Fusion trained on RAVDESS (14→32→16→7).

Same pipeline as deepface_audio_fusion_demo.py but uses the trained
MLP model instead of V3 rule-based probability fusion.

Usage:
    PYTHONPATH=. python demos/mlp_fusion_demo.py --camera 1
"""
...
class DeepFaceAudioFusionApp:
    ...
    def __init__(self, camera_index: int = 0):
        ...
        # Fusion (MLP learned)
        self.fusion = MLPFusion(model_path='models/mlp_fusion.pt')
        ...
        # GUI
        self.root = tk.Tk()
        self.root.title("DeepFace + Emotion2Vec MLP Fusion")
    ...
    def _setup_ui(self):
        ...
        tk.Label(top, text="DeepFace + Emotion2Vec MLP Fusion",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='DeepFace + Emotion2Vec MLP Fusion Demo')
